...
void tkey_destroy(tkey_handle_t key)
{
    if (!key)
        tkey_free(key);
    key = NULL;
...
void tkey_handler(tkey_handle_t key)
{
    ...
    {
        if (key->detect_cb() == key->pressed_level)
        {
            if (key->pressed_ticks >= key->debounce_ticks)
            {
                ...
                if (key->multi_press_count > 1)
                    key->event_cb(key, TKEY_EVENT_MULTI_PRESS, key->multi_press_count, key->user_data);
                else
                    key->event_cb(key, TKEY_EVENT_PRESS, key->multi_press_count, key->user_data);
            }
    ...
    else if (key->press_state == TKEY_STATE_PRESSED)
    {
        ...
        if (key->detect_cb() != key->pressed_level)
        {
            ...
            if (key->multi_press_count > 1)
                key->event_cb(key, TKEY_EVENT_MULTI_RELEASE, key->multi_press_count, key->user_data);
            else if (key->long_pressed_flag)
            {
                key->long_pressed_flag = 0;
                key->event_cb(key, TKEY_EVENT_LONG_RELEASE, key->multi_press_count, key->user_data);
            }
            else
                key->event_cb(key, TKEY_EVENT_RELEASE, key->multi_press_count, key->user_data);
            key->pressed_ticks = 0;
        }
        else if (key->pressed_ticks == key->hold_ticks)
        {
            key->long_pressed_flag = 1;
            key->event_cb(key, TKEY_EVENT_LONG_PRESS, key->multi_press_count, key->user_data);
        }
    }
}

void tkey_register_cbs(tkey_handle_t key, tkey_event_cb_t event_cb, tkey_detect_cb_t detect_cb, void *user_data)
{
    if (!key)
        return;
    key->init_ok_flag = 0;
...
tkey_handle_t tkey_create_default(void)
{
    ...
    init.event_cb = NULL;
    init.hold_ticks = 100;
    init.multi_press_interval_ticks = 25;
